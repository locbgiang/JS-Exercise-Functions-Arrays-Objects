...
console.log(makePersonObject(5, 'Leia', 'liea@liea.com'));
...
function makeSmartPerson(nameIn) {
  ...
  let obj = {
    ...
    speak: function(){
      return 'Hello, my name is '+nameIn;
    }
  }
  return obj;
}
console.log(makeSmartPerson('Loc'));
...
function getCarInfoById(inventory,id) {
  ...
  return 'This is a '+inventory[foundId].car_make+' '+inventory[foundId].car_model;
}
...
function getOlderCars(inventory, maxYear) {
  ...
  for(let i=0;i<inventory.length;i++){
    if(inventory[i].car_year<=maxYear){
      oldCars[oldCarsCounter] = inventory[i]
      oldCarsCounter++;
    }
  }
  return oldCars;
...
*/
function carMaker(numIn) {
  /* code here */
  const obj = {
    odometer: numIn,
    drive: function(distanceIn){
      this.odometer=this.odometer + distanceIn;
